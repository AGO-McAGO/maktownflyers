...
exports.sendForgotPasswordCode = async (request, response) => {
    ...
    const locals = {
      ...
      // return response.status(301).redirect("admin/adminresetpassword", { locals, layout: adminLayout } );
      // response.status(301).redirect("admin/adminresetpassword", { locals, layout: adminLayout } );
      // return response.status(301).redirect("admin/adminresetpassword", { locals, layout: adminLayout } );
      // return response.status(301, "admin/adminresetpassword", { locals, layout: adminLayout } );
      // return response.redirect("admin/adminresetpassword", { locals, layout: adminLayout } );
      response.redirect("admin/adminresetpassword", { locals, layout: adminLayout } );

      // return response.status(200).json({ message: "Code Sent! Please check your mail." });
      response.render('forgot-password.ejs', { csrfToken: req.csrfToken(), msg: "Reset email sent. Check your email for more info.", type: 'success' });
		}
...
exports.verifyForgotPasswordCode = async (request, response) => {
	const { adminname, providedCode, newPassword } = request.body; // get the Adminname, the forgot password code provided by the Admin and the Admin's new password

	try {
		const { error, value } = acceptFPCodeSchema.validate( { adminname, providedCode, newPassword } ); // validate the Admin's inputs
		...
		//** if there's no error ...
		const codeValue = providedCode.toString(); // convert the provided code to a string value and store/hold
		...
		if ( !existingAdmin.forgotPasswordCode || !existingAdmin.forgotPasswordCodeValidation ) { // if these do not exist ...
			return response.status(400).json({ success: false, message: "Sorry! Error with the code!" }); // display error message
		}
		...
		if (hashedCodeValue === existingAdmin.forgotPasswordCode) { // if user provided hashed code value is the same with forgot password code in the database ...
			const hashedPassword = await doHash(newPassword, 10); // then, hash the user's new password
